...
export function NativeSelect({ className, options, ...props }: React.ComponentProps<"select"> & { options: readonly string[] }) {
  return (
    <select
      className={cn("h-9 w-full rounded-md border border-input bg-transparent px-2 text-sm shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 dark:bg-input/30", className)}
      {...props}
    >
